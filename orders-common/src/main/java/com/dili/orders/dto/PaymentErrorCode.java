...
public enum PaymentErrorCode {
	...
	ACCOUNT_PASSWORD_INCORRECT_EXCEPTION("503003", "账号密码不正确"),
	BUYER_PASSWORD_INCORRECT_EXCEPTION("5030031", "买方密码不正确"),
	SELLER_PASSWORD_INCORRECT_EXCEPTION("5030032", "卖方密码不正确"),
	BUYER_SELLER_PASSWORD_INCORRECT_EXCEPTION("50300312", "买方和卖方密码不正确"),
	LACK_OF_BALANCE_EXCEPTION("503004", "账户余额不足");
	...
	public static PaymentErrorCode codeOf(String code) {
		for (PaymentErrorCode error : PaymentErrorCode.values()) {
			if (error.getCode().equals(code)) {
				return error;
			}
		}
		throw new IllegalArgumentException("位置错误码");
	}
