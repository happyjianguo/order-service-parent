...
//        BaseOutput<String> sg_zlc_settlement = uidRpc.bizNumber("sg_zlc_settlement");
	...
	private void setSellerCustomerMarketType(TransitionDepartureSettlement settlement, CustomerExtendDto customer) {
		CharacterType sellerType = customer.getCharacterTypeList().stream().filter(c -> c.getCharacterType().equals(CustomerEnum.CharacterType.经营户.getCode())).findFirst().orElse(null);
		if (sellerType == null) {
			throw new AppException("该卡客户身份类型不符");
		}
		settlement.setCustomerMarketTypeCode(sellerType.getSubType());
	}
