...
/**
 * 由MyBatis Generator工具自动生成 This file was generated on 2020-06-17 08:51:33.
 */
@Service
...
//        if (!Objects.equals(collectionRecord.getAmountActually(), 0L)) {
		...
		PaymentTradeCommitResponseDto data = null;
		if (collectionRecord.getAmountActually() > 0) {
			// 先创建预支付，再调用支付接口
...
//        collectionRecord.setAccountBuyerId(buyerAccountInfo.getAccountId());
		...
		// 买家设置期初余额
		buyerSerialRecordDo.setStartBalance(data != null ? data.getBalance() - data.getFrozenBalance() : buyerAccountFund.getAvailableAmount());
		// 买家设置期末余额
		buyerSerialRecordDo.setEndBalance(data != null ? data.getBalance() + data.getAmount() - data.getFrozenBalance() : buyerAccountFund.getAvailableAmount());
		// 买家设置操作时间
		buyerSerialRecordDo.setOperateTime(now);
		// 买家设置动作
		buyerSerialRecordDo.setAction(ActionType.EXPENSE.getCode());
		// 设置买家卡，持卡人姓名
		buyerSerialRecordDo.setHoldName(buyerAccountInfo.getHoldName());

		BalanceResponseDto sellerAccountFund = sellerAccountSimple.getData().getAccountFund();

		// 卖家设置交易流水code
		sellerSerialRecordDo.setTradeNo(collectionRecord.getPaymentNo());
		// 卖家设置期初金额
		sellerSerialRecordDo.setStartBalance(data != null ? data.getRelation().getBalance() - data.getRelation().getFrozenBalance() : sellerAccountFund.getAvailableAmount());
		// 卖家设置期末余额
		sellerSerialRecordDo
				.setEndBalance(data != null ? data.getRelation().getBalance() + data.getRelation().getAmount() - data.getRelation().getFrozenBalance() : sellerAccountFund.getAvailableAmount());
		// 卖家设置操作时间
		sellerSerialRecordDo.setOperateTime(now);
		// 卖家设置动作
		sellerSerialRecordDo.setAction(ActionType.INCOME.getCode());
		// 设置卖家卡，持卡人
