...
public class WeighingBillServiceImpl extends BaseServiceImpl<WeighingBill, Long> implements WeighingBillService {
    ...
    @Autowired
    private OrderSourceSyncMapper orderSourceSyncMapper;
    @Autowired
    private QualityTraceRpc qualityRpc;
    ...
    @Transactional
    @Override
    public BaseOutput<Object> sourceSync() {
        List<WeighingBillClientListDto> list = this.getActualDao().listSourceSyncData();
        if (CollectionUtils.isNotEmpty(list)) {
            BaseOutput<Object> output = this.qualityRpc.orderGoodsDetection(list);
            if (!output.isSuccess()) {
                LOGGER.error(output.getMessage());
                return BaseOutput.failure("调用朔源同步订单商品检测值失败");
            }
            List<Long> ids = new ArrayList<>(list.size());
            list.forEach(o -> ids.add(o.getId()));
            Example example = new Example(OrderSourceSync.class);
            example.createCriteria().andIn("orderId", ids);
            int rows = this.orderSourceSyncMapper.deleteByExample(example);
            if (rows < list.size()) {
                throw new AppException("删除同步数据失败");
            }
        }
        return BaseOutput.success();
    }
    ...
    public BaseOutput<WeighingStatement> settle(Long id, String buyerPassword, Long operatorId, Long marketId) {
        ...
        //插入朔源同步数据表定时同步
        OrderSourceSync oss = new OrderSourceSync();
        oss.setOrderId(weighingBill.getId());
        rows = this.orderSourceSyncMapper.insert(oss);
        if (rows <= 0) {
            throw new AppException("保存订单朔源同步数据失败");
        }
