...
public class WeighingBillServiceImpl extends BaseServiceImpl<WeighingBill, Long> implements WeighingBillService {
    ...
    @Qualifier("weighingBillSerialNoGenerator")
    @Autowired
    protected SerialNumberGenerator weighingBillSerialNoGenerator;
    @Qualifier("weighingStatementSerialNoGenerator")
    @Autowired
    ...
    @Autowired
    private AccountStatementRecorder accountStatementRecorder;
    ...
    public BaseOutput<WeighingStatement> addWeighingBill(WeighingBill bill) {
        ...
        User operator = this.userLoader.getById(bill.getCreatorId());
    ...
    public BaseOutput<WeighingStatement> freeze(Long id, String buyerPassword, Long operatorId) {
        ...
        long start = System.currentTimeMillis();
        AccountBalanceDto balanceResult = this.paymentService.queryBalance(weighingBill.getBuyerAccount());
        long end = System.currentTimeMillis();
        LOGGER.debug("结束调用支付系统查询买方余额，过磅单id：{}，耗时{}毫秒", weighingBill.getId(), end - start);
        if (balanceResult.getAvailableAmount() < weighingStatement.getFrozenAmount()) {
            return BaseOutput.failure(String.format("买方卡账户余额不足，还需充值%s元", MoneyUtils.centToYuan(weighingStatement.getFrozenAmount() - balanceResult.getAvailableAmount())));
        }

        // 冻结交易
        PaymentTradeCommitResponseDto freezePayResult = this.paymentService.freeze(weighingStatement, buyerPassword);
        LocalDateTime now = LocalDateTime.now();
        ...
        User operator = this.userLoader.getById(operatorId);
        weighingStatement.setLastOperationTime(freezePayResult.getWhen());
        ...
        // 记录操作流水
        WeighingBillOperationRecord wbor = this.buildOperationRecord(weighingBill, weighingStatement, operator, WeighingOperationType.FREEZE, freezePayResult.getWhen());
        rows = this.wbrMapper.insertSelective(wbor);
        if (rows <= 0) {
            throw new AppException("保存操作记录失败");
        }
        ...
        // 记账冻结流水
        this.accountStatementRecorder.recordFreeze(weighingStatement, freezePayResult);
    ...
    public PrintTemplateDataDto<WeighingStatementPrintDto> getWeighingStatementPrintData(String serialNo) {
        ...
        // 查询买方余额
        AccountBalanceDto accountBalanceDto = this.paymentService.queryBalance(wb.getBuyerAccount());
        dto.setBuyerBalance(accountBalanceDto.getAvailableAmount());
    ...
    public BaseOutput<Object> invalidate(Long id, String buyerPassword, String sellerPassword, Long operatorId) {
        ...
        boolean success = this.paymentService.validateAccountPassword(weighingBill.getBuyerAccount(), buyerPassword);
        if (!success) {
            buyerError = PaymentErrorCode.BUYER_PASSWORD_INCORRECT_EXCEPTION;
        }

        success = this.paymentService.validateAccountPassword(weighingBill.getSellerAccount(), sellerPassword);
        if (!success) {
        ...
        return this.operatorInvalidate(id, operatorId);
    }
    ...
    public BaseOutput<Object> operatorInvalidate(Long id, Long operatorId) {
        ...
        PaymentTradeCommitResponseDto refoundResult = null;
        // 解冻
        if (beforeUpdateState.equals(WeighingBillState.FROZEN.getValue())) {
            ...
            wsQuery.setWeighingBillId(weighingBill.getId());
            WeighingStatement weighingStatement = this.weighingStatementMapper.selectOne(wsQuery);
            refoundResult = this.paymentService.refound(weighingStatement.getPayOrderNo());
        }

        User operator = this.userLoader.getById(operatorId);
        LocalDateTime operationTime = null;
        if (refoundResult != null) {
            operationTime = refoundResult.getWhen();
        } else {
            operationTime = now;
        }
        ...
        if (refoundResult != null) {
            this.accountStatementRecorder.recordInvalidate(ws, refoundResult);
        }
    ...
    public BaseOutput<Object> operatorWithdraw(Long id, Long operatorId) {
        ...
        // 退款
        PaymentTradeCommitResponseDto refoundResult = this.paymentService.refound(weighingStatement.getPayOrderNo());

        // 重新生成一条过磅单
        this.rebuildWeighingBill(refoundResult, weighingBill, operatorId);

        User operator = this.userLoader.getById(operatorId);
        weighingStatement.setState(WeighingStatementState.REFUNDED.getValue());
        weighingStatement.setModifierId(operatorId);
        weighingStatement.setModifiedTime(refoundResult.getWhen());
        weighingStatement.setLastOperationTime(refoundResult.getWhen());
        weighingStatement.setLastOperatorId(operatorId);
        weighingStatement.setLastOperatorName(operator.getRealName());
        weighingStatement.setLastOperatorUserName(operator.getUserName());
        rows = this.weighingStatementMapper.updateByPrimaryKeySelective(weighingStatement);
        ...
        // 记录操作流水
        WeighingBillOperationRecord wbor = this.buildOperationRecord(weighingBill, weighingStatement, operator, WeighingOperationType.WITHDRAW, refoundResult.getWhen());
        rows = this.wbrMapper.insertSelective(wbor);
        if (rows <= 0) {
            throw new AppException("保存操作记录失败");
        }
        ...
        // 记录交易流水
//        this.recordWithdrawAccountFlow(operatorId, refoundResult, weighingBill, weighingStatement);
        this.accountStatementRecorder.recordWithdraw(weighingStatement, refoundResult);
    ...
    public BaseOutput<WeighingStatement> settle(Long id, String buyerPassword, Long operatorId, Long marketId) {
        ...
        // 判断余额是否足够
        AccountBalanceDto accountBalanceDto = this.paymentService.queryBalance(weighingBill.getBuyerAccount());
        Long banlance = accountBalanceDto.getAvailableAmount();
        ...
        if (banlance < weighingStatement.getBuyerActualAmount()) {
            return BaseOutput.failure(String.format("买方卡账户余额不足，还需充值%s元", MoneyUtils.centToYuan(weighingStatement.getBuyerActualAmount() - accountBalanceDto.getAvailableAmount())));
        }
        ...
        PaymentTradeCommitResponseDto paymentResult = null;
        if (weighingBill.getState().equals(WeighingBillState.FROZEN.getValue())) {
            paymentResult = this.paymentService.freezePay(weighingStatement, buyerPassword);
        } else {
            paymentResult = this.paymentService.pay(weighingStatement, buyerPassword);
        }
        ...
        // 设置代理人信息
        this.updateAgentInfo(weighingBill, weighingStatement);

        User operator = this.userLoader.getById(operatorId);
        weighingStatement.setLastOperationTime(paymentResult.getWhen());
        weighingStatement.setLastOperatorId(operatorId);
        weighingStatement.setLastOperatorName(operator.getRealName());
        weighingStatement.setLastOperatorUserName(operator.getUserName());
        ...
        weighingBill.setSettlementTime(paymentResult.getWhen());
        rows = this.getActualDao().updateByPrimaryKeySelective(weighingBill);
        if (rows <= 0) {
        ...
        // 记录操作流水
        WeighingBillOperationRecord wbor = this.buildOperationRecord(weighingBill, weighingStatement, operator, WeighingOperationType.SETTLE, paymentResult.getWhen());
        rows = this.wbrMapper.insertSelective(wbor);
        if (rows < 0) {
            throw new AppException("保存操作记录失败");
        }
        ...
        // 记录资金账户交易流水
        this.accountStatementRecorder.recordSettlement(weighingStatement, paymentResult);
        // 发送mq通知中间价计算模块计算中间价
    ...
    protected void updateAgentInfo(WeighingBill weighingBill, WeighingStatement weighingStatement) {
    ...
    public BaseOutput<WeighingStatement> updateWeighingBill(WeighingBill dto) {
        ...
        User operator = this.userLoader.getById(weighingBill.getModifierId());
        LocalDateTime now = LocalDateTime.now();
    ...
    public BaseOutput<Object> withdraw(Long id, String buyerPassword, String sellerPassword, Long operatorId) {
        ...
        this.paymentService.validateAccountPassword(weighingBill.getSellerAccount(), sellerPassword);
        if (success) {
            sellerError = PaymentErrorCode.SELLER_PASSWORD_INCORRECT_EXCEPTION;
        }
        ...
        return this.operatorWithdraw(id, operatorId);
    }
