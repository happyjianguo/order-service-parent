...
//		BaseOutput<String> output = this.uidRpc.getCode();
        BaseOutput<String> stringBaseOutput = this.uidRpc.bizNumber("sg_weighing_bill");
        if (!stringBaseOutput.isSuccess()) {
            LOGGER.error(stringBaseOutput.getMessage());
            return BaseOutput.failure("生成过磅单编号失败");
        }
        weighingBill.setSerialNo(stringBaseOutput.getData());
...
//        BaseOutput<String> output = this.uidRpc.getCode();
        BaseOutput<String> stringBaseOutput = this.uidRpc.bizNumber("sg_weighing_statement");
        if (!stringBaseOutput.isSuccess()) {
            throw new AppException(stringBaseOutput.getMessage());
        }
        WeighingStatement ws = new WeighingStatement();
        ws.setSerialNo(stringBaseOutput.getData());
